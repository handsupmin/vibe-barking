...
import { SUPPORTED_CATEGORIES, SUPPORTED_PROVIDERS, type PublicJobRecord, type QueueEnqueueInput } from './types.ts';
...
export function createApp({ providers, queue, env = process.env, fetchFn = fetch, cwd = process.cwd() }: CreateAppOptions = {}) {
  const registry = providers ?? createProviders({ env, fetchFn, cwd });
  const providerMap = new Map(registry.map((provider) => [provider.id, provider]));
  const jobQueue =
    queue ??
    new JobQueue({
  ...
  return {
    ...
    async fetch(request: Request): Promise<Response> {
      ...
      if (request.method === 'POST' && url.pathname === '/api/providers/validate') {
        const body = await readJson<{ provider?: string; model?: string }>(request);
        if (!body.provider || !providerMap.has(body.provider as any)) {
          return json({ error: 'Unknown provider.' }, { status: 400 });
        }

        const result = await providerMap.get(body.provider as any)!.validate({
          model: body.model,
        });
      ...
      if (request.method === 'POST' && url.pathname === '/api/jobs') {
        ...
        if (!body.chunk || typeof body.chunk !== 'string' || body.chunk.trim().length === 0) {
          return json({ error: 'Chunk is required.' }, { status: 400 });
        }

        const job = jobQueue.enqueue({
          provider: body.provider as QueueEnqueueInput['provider'],
          chunk: body.chunk,
...
function toPublicJob(job: ReturnType<JobQueue['get']> extends infer T ? Exclude<T, undefined> : never): PublicJobRecord {
  return {
    id: job.id,
