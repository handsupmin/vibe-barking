...
export function createClaudeProvider({ env = process.env, fetchFn = fetch }: ClaudeProviderOptions = {}): ProviderAdapter {
  ...
  return {
    ...
    displayName,
    configSummary() {
      const missing = requiredEnv(env, ['ANTHROPIC_API_KEY', 'ANTHROPIC_MODEL']);
      return {
        provider: 'claude',
        displayName,
...
async function validateClaude({
  ...
  const missing = requiredEnv({ ...env, ANTHROPIC_MODEL: effectiveModel }, ['ANTHROPIC_API_KEY', 'ANTHROPIC_MODEL']);
  if (missing.length > 0) {
    return {
      ok: false,
      provider: 'claude',
  ...
  try {
    const response = await fetchClaude(fetchFn, env.ANTHROPIC_API_KEY!, effectiveModel!, {
    ...
    return {
      ...
      provider: 'claude',
      model: effectiveModel,
      message: error instanceof Error ? error.message : 'Claude validation failed.',
    };
  }
}
