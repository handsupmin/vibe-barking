...
export function createCodexCliProvider({ env = process.env, cwd = process.cwd() }: CodexCliProviderOptions = {}): ProviderAdapter {
...
async function validateCodex({
  ...
  try {
    ...
    return {
      ...
      provider: 'codex',
      model: model ?? env.CODEX_MODEL,
      message: /ready/i.test(outputText) ? 'Codex CLI responded successfully.' : 'Codex CLI ran, but validation text was unexpected.',
    };
  } catch (error) {
    return {
      ok: false,
      provider: 'codex',
      model: model ?? env.CODEX_MODEL,
      message: error instanceof Error ? error.message : 'Codex CLI validation failed.',
    };
  }
}
...
async function runCodexPrompt({
  ...
  try {
    ...
    if (result.code !== 0) {
      throw new Error(result.stderr.trim() || `Codex CLI exited with code ${result.code}.`);
    }
