import { DEFAULT_CATEGORY, SUPPORTED_CATEGORIES, type PromptCategory, type PromptFrame } from '../types.ts';
...
export function normalizeCategory(category?: string): PromptCategory {
  ...
  return (SUPPORTED_CATEGORIES as readonly string[]).includes(category) ? (category as PromptCategory) : DEFAULT_CATEGORY;
}

export function framePrompt({ chunk, category, sequence }: FramePromptInput): PromptFrame {
