...
test('buildCodexCommand returns a fixed safe codex exec invocation', () => {
  ...
  assert.deepEqual(command.args.slice(0, 5), ['exec', '--skip-git-repo-check', '--sandbox', 'read-only', '--output-last-message']);
...
test('buildCodexCommand rejects unsafe executable strings', () => {
  assert.throws(() => buildCodexCommand({ codexPath: 'codex; rm -rf /' }), /unsafe/i);
});
